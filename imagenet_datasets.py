...
class ImageNet:
    def __init__(self,
                 preprocess,
                #  location=os.path.expanduser('~/data'), #CHANGED#
                location='/scratch3/workspace/oraundale_umass_edu-quadratic-ensembling/model-soups/data',
                 batch_size=32,
                 num_workers=32,
                 classnames='openai',
                 distributed=False):
    ...
    def populate_train(self):
        ...
        if os.path.isdir(traindir):
            ...
            # traindir = os.path.join('/datasets/ai/imagenet/object-detection/ILSVRC/Data/CLS-LOC', self.name(), 'train')
            traindir = os.path.join('/datasets/ai/imagenet/object-detection/ILSVRC/Data/CLS-LOC', 'train')
        ##CHANGED##
    ...
    def get_test_path(self):
        ...
        if os.path.isdir(test_path):
            print("Directory exists")
        # else:
        #     test_path = os.path.join('/data', self.name(), 'val_in_folder')
        
        if not os.path.exists(test_path):
            # test_path = os.path.join(self.location, self.name(), 'val') ##CHANGED##
            test_path = os.path.join(self.location, self.name(), 'val')
            if os.path.isdir(test_path):
                print("Directory exists")   
            # else:
                # test_path = os.path.join('/datasets/ai', self.name(), 'val')
